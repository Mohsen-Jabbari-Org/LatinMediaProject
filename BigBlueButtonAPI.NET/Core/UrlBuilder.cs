...
namespace BigBlueButtonAPI.Core
{
    ...
    {
        private BigBlueButtonAPISettings settings;
        ...
        private string Build(string method, string parameters, bool onlyQueryString = false)
        {
            ...
            } else
            {
                if (string.IsNullOrEmpty(parameters))
                {
                    return string.Format("{0}{1}?checksum={2}", BigBlueButtonAPISettings.ServerAPIUrl, method, checksum);
                }
                else
                {
                    return string.Format("{0}{1}?{2}&checksum={3}", BigBlueButtonAPISettings.ServerAPIUrl, method, parameters, checksum);
                }
            }
        ...
        {
            return BigBlueButtonAPISettings.ServerAPIUrl + method;
        }
        /// <summary>
        ...
        {
            if (parameters == null) parameters = string.Empty;
            return HashHelper.Sha1Hash(method + parameters, BigBlueButtonAPISettings.SharedSecret);
        }
    }
}