...
namespace LatinMedia.Web.Pages.Leon.SurvayDetailReport
{
    //[UserRoleChecker]
    //[PermissionChecker(49)]
    public class SurvayDetailReportModel : PageModel
    {
        private ICourseService _courseService;
        private IUserService _userService;

        public SurvayDetailReportModel(ICourseService courseService, IUserService userService)
        {
        ...
        [BindProperty]
        public List<ReportForSurvayDetailTeachersViewModel> ReportForSurvayDetailTeachersViewModels { get; set; }

        public List<ReportForSurvayDetailTeachersViewModel> Tmodel { get; set; }
...

        {
            ...
            ReportForSurvayDetailTeachersViewModels = new List<ReportForSurvayDetailTeachersViewModel>();
            if (!string.IsNullOrEmpty(stDate) && !string.IsNullOrEmpty(fhDate))
            {
                ...
                int[] AcademyList = _courseService.GetTeachersAcademyForSurvay(ReportForTeacherViewModel);
                for (int i = 0; i < AcademyList.Length; i++)
                {
                    Tmodel = _courseService.GetReportForTeacherSurvay(paging, ReportForTeacherViewModel, AcademyList[i], pageId, take);
            ...
            else
            {
                ...
                for (int i = 0; i < AcademyList.Length; i++)
                {
                    ReportForSurvayDetailTeachersViewModels = _courseService.GetReportForTeacherSurvay(paging, ReportForTeacherViewModel, AcademyList[i], pageId, take);
                }
                Genders = _userService.GetGenders();
                ViewData["Gender"] = new SelectList(Genders, "GenderId", "GenderName");
                Educations = _userService.GetEducations();
