...
namespace LatinMedia.Web.Pages.Academy.RhgAcClass
{
    public class CreateClassModel : PageModel
    {
        ...
        public CreateClassModel(ICourseService courseService, IUserService userService, BigBlueButtonAPIClient client)
        {
        ...
        public IActionResult OnPost(IFormFile courseFile, IFormFile imgCourseUp , string stDate = "", string fhDate = "", int AcademyId = 0)
        {
            ...
            Course.CourseTime = 150;
            Course.MaxUsers = _userService.GetMaxUsersForAcademies(AcId);
            if (Course.VTA_Id == 0 || Course.TeacherId == 0)
            {
                var groups = _courseService.GetGroupsForManageCourse();
                ViewData["Groups"] = new SelectList(groups, "Value", "Text");
            ...
            #endregion
            //var server = _courseService.GetServerParameters(AcId);
            Course.ServerId = 2;
