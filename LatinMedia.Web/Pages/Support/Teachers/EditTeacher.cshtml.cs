    ...
    public class EditTeacherModel : PageModel
    {
        ...
        public IActionResult OnPost(List<int> selectedTypes)
        {
            ...
            _userService.AddGeneralEvent(Convert.ToInt32(User.Identity.GetID()), 5, DateTime.Now, remoteIpAddress.ToString(),
                " اطلاعات مدرس " + EditTeacherViewModel.FirstName + " " + EditTeacherViewModel.LastName + " به شماره همراه " + EditTeacherViewModel.Mobile +
                " توسط " + User.Identity.Name + " بروز رسانی گردید");
            return RedirectToPage("Index");
        }
    }
}